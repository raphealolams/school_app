...
class Admin extends CI_Controller
{
    ...
    /******* Manage Sections**********/
    function school_sections($param1 = '', $param2 = '' , $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/school_sections/', 'refresh');
        }
        else {
          $page_data['page_name']  = 'sections';
          $page_data['section']   = $this->db->get_where('school_section')->result_array();
          $page_data['page_title'] = 'School Section Configuration';
          $this->load->view('backend/index', $page_data);
        }
      }

      /******* Manage Terms**********/
      function school_term($param1 = '')
      {
          if ($this->session->userdata('admin_login') != 1)
              redirect(base_url(), 'refresh');
          if ($param1 == 'create') {
              ...
              $this->session->set_flashdata('flash_message' , 'New Term Added Successfully');
              redirect(base_url() . 'index.php?admin/school_term/', 'refresh');
          }
          else {
            $page_data['page_name']  = 'term';
            $page_data['term']   = $this->db->get_where('term')->result_array();
            $page_data['page_title'] = 'School Term Configuration';
            $this->load->view('backend/index', $page_data);
          }
      }
