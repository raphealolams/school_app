...
class Teacher extends CI_Controller
{
    ...
    /****MANAGE EXAM MARKS*****/
    function reports($section_id = '',$term_id = '', $class_id = '', $subject_id = '')
    {
        if ($this->session->userdata('teacher_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['section_id'] > 0 && $page_data['term_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'index.php?teacher/reports/' . $page_data['section_id'] . '/' . $page_data['term_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
                redirect(base_url() . 'index.php?teacher/reports/', 'refresh');
            }
        }
        if ($this->input->post('operation') == 'update') {
            $students = $this->db->get_where('student' , array('class_id' => $class_id))->result_array();
            foreach($students as $row) {
                $data['exam_score'] = $this->input->post('exam_score_' . $row['student_id']);
                $data['assessment_score'] = $this->input->post('assessment_score_' . $row['student_id']);
                $data['comment']       = $this->input->post('comment_' . $row['student_id']);

                $this->db->where('report_id', $this->input->post('report_id_' . $row['student_id']));
                // echo "<pre>";
                // var_dump($this->db->where('report_id', $this->input->post('report_id_' . $row['student_id'])));
                // echo "</pre>";
                $this->db->update('report', array(
                  'exam_score' => $data['exam_score'],
                  'assesment_score' => $data['assessment_score'],
                  'status' => 0,
                  'comment' => $data['comment']));
            }
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?teacher/reports/' . $this->input->post('section_id') . '/' .$this->input->post('term_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('subject_id'), 'refresh');
        }
        $page_data['section_id'] = $section_id;
        $page_data['term_id']    = $term_id;
        $page_data['class_id']   = $class_id;
        $page_data['subject_id'] = $subject_id;
        $page_data['page_info']  = 'Report Processing';
